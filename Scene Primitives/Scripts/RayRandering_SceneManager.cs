...
namespace QuizCanners.RayTracing
{
    ...
    internal partial class Singleton_RayRendering
    {
        ...
        internal class SceneManager : IPEGI, ILinkedLerping, ICfgCustom, IPEGI_ListInspect, INeedAttention
        {
            ...
            protected Singleton_RayRendering Mgmt => Singleton.Get<Singleton_RayRendering>();

            public void OnSetBakingDirty() => StableFrames = 0;
            ...
            {
                if (TracingPrimitives)
                    TracingPrimitives.Lerp(ld, canSkipLerp);
            ...
            public void DecodeTag(string key, CfgData data)
            {
                switch (key)
                {
                    case "se": TracingPrimitives.Decode(data); break;
                    case "gm":
                        if (GodModeCamera)
                        {
