			...
			Pass
			{
				...
				*/
				...
				v2f vert(appdata_t v)
				{
					...
					o.shadow = SampleRayShadow(worldPos) * SampleSkyShadow(worldPos);
				...
				//	return  col;
					...
					float ao;
					float3 bake = SampleAmbientLight(i.worldPos, ao);


					float3 shadowPos = i.worldPos;// +volumetriEdge * viewDir.xyz;;// +i.viewDir.xyz * 2 * volumetriEdge;

					float shadow = i.shadow;
					//SampleRayShadow(shadowPos) * SampleSkyShadow(i.worldPos);

					float toSun = saturate(dot(_WorldSpaceLightPos0.xyz, -viewDir));

					//return toSun;

					col.rgb = bake * (0.25f + col.rgb * 0.75f) * ao + 
					col.rgb * 
