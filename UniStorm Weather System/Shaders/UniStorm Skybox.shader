...
SubShader 
{
	...
	Pass 
	{
		...
		v2f vert (appdata_t v)
		{
			...
			OUT.viewDir.xyz = WorldSpaceViewDir(v.vertex);
...
//				for(int i=0; i<int(kSamples); i++)
			...
			OUT.groundColor	= unity_FogColor; // _Exposure * (cIn * getRayleighPhase(_WorldSpaceLightPos0.xyz, -eyeRay));
			OUT.skyColor = _RayMarchSkyColor; // _Exposure* (cIn * getRayleighPhase(_WorldSpaceLightPos0.xyz, -eyeRay));
			...
			OUT.noiseUV = (samplePoint.xy + samplePoint.z ) * (123.12345678) + float2(_SinTime.x, _CosTime.y) * 32.12345612;
		...
		y = saturate(y);
			...
			float4 noise = (tex2Dlod(_Global_Noise_Lookup, float4(IN.noiseUV.xy, 0, 0)) - 0.5) * (1- y);

#		if _RAY_MARCH_STARS
			float3 viewDir = normalize(IN.viewDir.xyz);
			float2 uv = (viewDir.xz * (8 + viewDir.y * 3) + 100) + noise.rg * 0.002;

			IN.skyColor += AllStars(uv, 0.1, 1)  * smoothstep(0.02, 0.5, -viewDir.y);
#		endif

			// if we did precalculate color in vprog: just do lerp between them
		col = lerp(IN.skyColor, IN.groundColor, y);
			...
			//{
				...
				col += mie * IN.sunColor * (smoothstep(1.55, 1.45, y));
			//}
		#endif
		...
		#else
				col.rgb += (noise.rgb) * 0.0075;
		#endif

		//col = lerp(unity_FogColor, col, smoothstep(0,0.1,-y.y));
